let apikey = "2618a4ce773c01d44febc6087c180ad6";
let trailers = [];
let cat = "now_playing";
let lang = "en";
...
function getMovies(category, language) {
    ...
    fetch(`https://api.themoviedb.org/3/movie/${(category=="")?cat:category}?api_key=${apikey}&with_original_language=${(language=="")?lang:language}&page=1`)
...
function openTrailer(id) {
    fetch(`https://api.themoviedb.org/3/movie/${trailers[id]}/videos?api_key=5fb32448a60249d6da6e825ac28ba2a3`)
        .then((response) => response.json())
        .then((data) => {
            console.log(data);
            if (data.results.length == 0) {
                ...
                _("#trailer").src = `https://www.youtube.com/embed/${data.results[0].key}`;
